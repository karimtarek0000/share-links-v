...
export default defineEventHandler(async event => {
  try {
    ...
    // Delete the previous uploaded image first, if it exists
    ...
    if (existingProfile?.img) {
      const oldImagePath = extractImagePathFromUrl(existingProfile.img, userId)
      if (oldImagePath) {
    ...
    const { data, error } = await uploadToStorage(supabase, uniqueFilename, fileData)
    if (error) {
    ...
    return createError({
      ...
      statusMessage: err.statusMessage || err.message || 'An error occurred uploading the image',
    })
  }
})
...
function validateUserId(formData: any[]) {
  ...
  return userIdField?.data ? Buffer.from(userIdField.data).toString('utf-8') : null
}

function extractImagePathFromUrl(publicUrl: string, userId: string): string | null {
  if (!publicUrl || publicUrl.startsWith('blob:')) {
    return null
  }

  const cleanUrl = publicUrl.split('?')[0].split('#')[0].trim()
  if (!cleanUrl.includes('profile-images/')) {
    return null
  }

  const parts = cleanUrl.split('profile-images/')
  if (parts.length > 1) {
...
async function uploadToStorage(supabase: any, uniqueFilename: string, fileData: any) {
  return supabase.storage.from('profile-images').upload(uniqueFilename, fileData.data, {
...
function getPublicUrl(supabase: any, uniqueFilename: string) {
  const { data } = supabase.storage.from('profile-images').getPublicUrl(uniqueFilename)
